...
def rewrite_bytecode(bc):
  ...
  for i in bc:
    ...
    if i.name == 'COMPARE_OP' and i.arg == bytecode.Compare.IN:
      newinstr.append(bytecode.Instr("LOAD_GLOBAL", "__rewriter_in"))
      newinstr.append(bytecode.Instr("ROT_THREE"))
      newinstr.append(bytecode.Instr("CALL_FUNCTION", 2))
      changed = True
      continue

    if i.name == 'COMPARE_OP' and i.arg == bytecode.Compare.NOT_IN:
      newinstr.append(bytecode.Instr("LOAD_GLOBAL", "__rewriter_not_in"))
      newinstr.append(bytecode.Instr("ROT_THREE"))
      newinstr.append(bytecode.Instr("CALL_FUNCTION", 2))
      changed = True
      continue
