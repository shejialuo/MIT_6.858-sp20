...
def clean_state(containers):
    for cn in containers:
    	c = lxc.Container(cn)
    	if c.running:
       	    c.attach_wait(lxc.attach_run_command, ["rm", "-rf", "/home/student/zoobar/db/"])
...
def check_dbexists(cn, name):
    c = lxc.Container(cn)
    if c.attach_wait(lxc.attach_run_command, ["test", "-e", "/home/student/zoobar/db/%s" % name]) != 0:
...
def check_nocomm(src, dst):
    s = lxc.Container(src)
    d = lxc.Container(dst)
    if s.attach_wait(lxc.attach_run_command, ["ping", "-nq", "-c", "5", "-i", "0.1", "-W", "1", ipaddr(d)]) == 0:
...
def check_fs():
    ...
    if not file_write_check("Exercise 11: /testfile write:", tmpfile, data,
                            "test1", "supersecretpassword", "test2", "pass"):
        fail("Exercise 11: /testfile check (could not write to /testfile)")
        return

    # try to read the same file
    if file_read_check("Exercise 11: shared /testfile:", tmpfile, None, data,
                       "test2", "pass", "test1", "supersecretpassword"):
        fail("Exercise 11: /testfile check (/testfile shared by more than one user)")
        return
    ok("Exercise 11: /testfile check")
